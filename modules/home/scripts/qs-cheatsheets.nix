...
pkgs.writeShellScriptBin "qs-cheatsheets" ''
  ...
  EOF
    ...
    if [[ "$CATEGORY" != "root" && ! -d "$HOME/nix-config/cheatsheets/$CATEGORY" ]]; then
      echo "Error: Category directory '$CATEGORY' not found in cheatsheets/" >&2
      echo "Available categories:" >&2
      echo "  root" >&2
      ls -1 "$HOME/nix-config/cheatsheets/" | grep -E '^[a-z]' | head -10 >&2
      exit 1
    fi
    ...
    if [ -d "$HOME/nix-config/cheatsheets" ]; then
      for category_dir in "$HOME/nix-config/cheatsheets"/*/; do
  ...
  Window {
    ...
    function loadFileContent(filename) {
      ...
      if (selectedCategory === "root") {
        filePath = "${config.home.homeDirectory}/nix-config/cheatsheets/" + filename;
      } else {
        filePath = "${config.home.homeDirectory}/nix-config/cheatsheets/" + selectedCategory + "/" + filename;
      }
